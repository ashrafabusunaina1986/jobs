...
function JobList({ filterCategories, user, profileInfo, jobs, jobApplicants }) {
    ...
    const handleFilter = (getCurrentId, getCurrentOption) => {
        ...
        if (indexOfFilter === -1) {
            ...
            const indexCurrentOption = cpyFilterPararms[getCurrentId].indexOf(getCurrentOption)
            if (indexCurrentOption === -1) cpyFilterPararms[getCurrentId].push(getCurrentOption)
    ...
    const filterMenus = filterMenuArray.map(filterItem => ({
        ...
        name: filterItem.l,
        options: [
            ...new Set(filterCategories?.map(filterCategory => filterCategory[filterItem?.id]))
        ]
    }))
    useEffect(() => {
        ...
        <div className='w-full'>
            <div className='flex flex-col md:flex-row gap-10 items-baseline md:justify-between  pt-20 border-b-[2px] border-gray-500 pb-6'>
                ...
                </h1>
                {
                    profileInfo?.role === 'recruiter' ?
                        ...
                        : <Menubar>
                            {
                                filterMenus.map((filterMenu, i) => <MenubarMenu key={i}>
                                    <MenubarTrigger className='cursor-pointer text-sm font-bold text-gray-500' >{filterMenu.name}</MenubarTrigger>
                                    <MenubarContent>
                                        {
                                            filterMenu.options.map((option, optionInd) => <MenubarItem onClick={() => handleFilter(filterMenu.id, option)} key={optionInd} className="flex items-center">
                                                <div className={`w-[14px] rounded text-blue-950 h-[14px] border-2 border-gray-950 ${filterParams && Object.keys(filterParams).length > 0 && filterParams[filterMenu.id] && filterParams[filterMenu.id].indexOf(option) > -1 ? 'bg-black' : ''} mx-2`} />
                                                <Label className=' cursor-pointer text-sm font-bold text-gray-500'>{option}</Label>
                                            </MenubarItem>)
                                        }
                                    </MenubarContent>
                                </MenubarMenu>)
            ...
            <div className='flex items-center justify-center w-[98%] mx-auto my-5'>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-5 px-8 py-5 w-full'>
                    {
                        jobs && jobs.map((job, i) => profileInfo?.role === 'recruiter' ? <RecruiterJobList key={i} job={job} profileInfo={profileInfo} jobApplicants={jobApplicants} /> : <CandidateJobList key={i} job={job} user={user} profileInfo={profileInfo} jobApplicants={jobApplicants} />)
                    }
                </div>
            </div>
        </div>
